...
class BaseWorker:
    ...
    def call_func(self, func: dict):
        try:
            if func.get('args') and func.get('kwargs'):
                returned = loads(func.get('func'))(*loads(func.get('args')), **loads(func.get('kwargs')))
            elif func.get('args'):
    ...
    def _random_id(self) -> Union[str, None]:
        ...
        sample = list(self.col.aggregate([
    ...
    def watch(self):
        try:
            with self.col.watch([{'$match': {'status': {'$in': [None, 'retry']}}}], batch_size=1) as stream:
                for _ in stream:
                    break
        except PyMongoError:
